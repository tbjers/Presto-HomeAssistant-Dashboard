...
- `dashboard/` — this project's own app code, layered on top of TmOS:
  ...
  - `topics.py` — the MQTT topic/payload contract. Single source of truth for topic strings and payload shapes; `mqtt_client.py` and tile code both import from here rather than hand-building either. Includes `device_config_topic`/`parse_config_payload` for the retained per-device screens/tiles config topic (see below).
...
- `config.py` — `DEVICE_ID` (needed locally, before any MQTT topic name can be built) plus `DEFAULT_SCREENS`, a minimal fallback shown until the device's real config arrives over MQTT. It is **not** the tile/entity registry anymore — see README.md's "Declare your tiles" section for the MQTT config-topic contract that replaced it. Tiles still address `dashboard.grid`'s 16-column coordinate system (a "standard" tile is `colspan=4, rowspan=4`).
