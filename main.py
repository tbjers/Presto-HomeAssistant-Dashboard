"""
PREVIEW BUILD -- not the final main.py.

Confirms grid layout, theme, and tile interactivity work end-to-end on
real hardware, ahead of dashboard/page.py, app.py, mqtt_client.py, and
config.py existing. There is no real MQTT broker here: LoopbackMQTT stands
in for dashboard.mqtt_client.DashboardMQTT -- publishing a /set command
feeds straight back into DashboardState, as if Node-RED round-tripped it
instantly, so taps visibly respond without a broker. This exact
state_store-driven wiring (mqtt.publish -> topics.parse -> state.set ->
tile.set_state) is what the real page.py will do for real.
"""
